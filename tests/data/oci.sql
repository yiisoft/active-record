...
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE "promotion_SEQ"'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END;--
...
CREATE TABLE "promotion" (
  ...
  "title" varchar2(126) NOT NULL,
  CONSTRAINT "promotion_PK" PRIMARY KEY ("id") ENABLE
);
...
END;
/
CREATE TRIGGER "promotion_TRG" BEFORE INSERT ON "promotion" FOR EACH ROW BEGIN <<COLUMN_SEQUENCES>> BEGIN
  IF INSERTING AND :NEW."id" IS NULL THEN SELECT "promotion_SEQ".NEXTVAL INTO :NEW."id" FROM SYS.DUAL; END IF;
END COLUMN_SEQUENCES;
END;
/
