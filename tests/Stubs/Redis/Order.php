...
final class Order extends ActiveRecord
{
    ...
    {
        return $this->hasMany(Item::class, ['id' => 'item_id'])->via('orderItemsWithNullFK');
    }
    ...
    {
        return $this->hasMany(Item::class, ['id' => 'item_id'])->via('orderItems', function ($q) {
            $q->where(['between', 'subtotal', 11, 1000]);
        });
    }
    ...
    {
        return $this->hasMany(Item::class, ['id' => 'item_id'])->via('orderItems', function ($q) {
            $q->where(['not between', 'subtotal', '1', '10'])->all();
        });
    }
