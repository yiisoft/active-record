...
trait ActiveRelationTrait
{
    ...
    private ActiveRecordModelInterface|null $primaryModel = null;
    /** @psalm-var string[] */
    private array $link = [];
    /**
     ...
     *
     * @return ActiveRecordModelInterface|array|null The related record(s).
     */
     ...
     *
     * @param ActiveRecordModelInterface[]|array[] $result the array of related records as generated by {@see populate()}
     *
     * @throws InvalidConfigException
     *
     * @psalm-param non-empty-list<ActiveRecordModelInterface|array> $result
     * @psalm-param-out non-empty-list<ActiveRecordModelInterface|array> $result
     */
    private function addInverseRelations(array &$result): void
    {
        ...
        if ($relatedModel instanceof ActiveRecordModelInterface) {
            ...
            /** @var ActiveRecordModelInterface $relatedModel */
            foreach ($result as $relatedModel) {
                $relatedModel->activeRecord()->populateRelation($this->inverseOf, $primaryModel);
            }
        } else {
            $inverseRelation = $this->getModelInstance()->relationQuery($this->inverseOf);
            $primaryModel = $inverseRelation->getMultiple() ? [$this->primaryModel] : $this->primaryModel;
    ...
    public function populateRelation(string $name, array &$primaryModels): array
    {
        ...
        if (!$this->multiple && count($primaryModels) === 1) {
            ...
            if ($primaryModel instanceof ActiveRecordModelInterface) {
                $primaryModel->activeRecord()->populateRelation($name, $models[0]);
            } else {
                /**
    ...
    private function populateRelationFromBuckets(
        ...
        foreach ($models as &$model) {
            ...
            if ($model instanceof ActiveRecordModelInterface) {
                $model->activeRecord()->populateRelation($name, $value);
            } else {
                /** @var array $model */
                $model[$name] = $value;
            }
    ...
    private function prefixKeyColumns(array $columnNames): array
    {
        if (!empty($this->join) || !empty($this->joinWith)) {
            if (empty($this->from)) {
                $alias = $this->getModelInstance()->tableName();
            } else {
    ...
    /**
     * @param ActiveRecordModelInterface[]|array[] $models
     *
     * @throws InvalidConfigException
     */
    protected function filterByModels(array $models): void
    {
        ...
        if (count($columnNames) === 1) {
            ...
            if ($model instanceof ActiveRecordModelInterface) {
                /** @var ActiveRecordModelInterface $model */
                foreach ($models as $model) {
                    $value = $model->activeRecord()->get($linkedProperty);
            ...
            match ($this->getModelInstance()->activeRecord()->columnType($propertyName)) {
        ...
        if ($model instanceof ActiveRecordModelInterface) {
            /** @var ActiveRecordModelInterface $model */
            foreach ($models as $model) {
                $value = $model->activeRecord()->propertyValues($this->link);
    ...
    private function getModelKeys(ActiveRecordModelInterface|array $model, array $properties): array
    {
        $key = [];
        ...
        if (is_array($model)) {
            ...
            foreach ($properties as $property) {
                $value = $model->activeRecord()->get($property);
    ...
    /**
     * @param ActiveRecordModelInterface[]|array[] $primaryModels either array of AR instances or arrays.
     *
     * @throws Exception
     * @throws Throwable
     * @throws \Yiisoft\Definitions\Exception\InvalidConfigException
    ...
    /**
     * @return ActiveRecordModelInterface|null the primary model of a relational query.
     *
     * This is used only in lazy loading with dynamic query options.
     */
    public function getPrimaryModel(): ActiveRecordModelInterface|null
    {
